...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	if(Stack_IsEmpty(stack)){							//pokial je zasobnik prazdny, vrati error
		Stack_Error(SERR_TOP);
		return;
	}
	else{
		*dataPtr=stack->array[stack->topIndex]; 		//vrati znak z vrcholu zasobnika
	}
    //solved = FALSE; /* V případě řešení, smažte tento řádek! */
}
...
void Stack_Pop( Stack *stack ) {
	if(!Stack_IsEmpty(stack)){							//pokial nie je zasobnik prazdny, odstrani prvok  vrcholu zasobnika 
		stack->topIndex--;
	}
    //solved = FALSE; /* V případě řešení, smažte tento řádek! */
}
...
void Stack_Push( Stack *stack, char data ) {
	if(Stack_IsFull(stack)){							//pokial je zasobnik plny, vrati error
		Stack_Error(SERR_PUSH);
		return;
	}
	else{
		stack->topIndex++;								//vlozi znak na vrchol zasobnika
